...
path = r"C:\Agrinova hackathon\landroid\landroid\src\services\signals.ts"
...
async function fetchNDVI(): Promise<Partial<SignalState>> {
  ...
  try {
    const res = await axios.post(
      'https://planetarycomputer.microsoft.com/api/stac/v1/search',
      {
        ...
        datetime: getDateRange(3), // 3 years
        limit: 100,
...
async function fetchRainfall(): Promise<Partial<SignalState>> {
  const { minLon, minLat, maxLon, maxLat } = getParcelCoords();
  try {
    const res = await axios.post(
      'https://planetarycomputer.microsoft.com/api/stac/v1/search',
...
export async function fetchAllSignals(): Promise<SignalState & { healthScore: number; healthLabel: string }> {
  const [soil, nom, osm, ndvi, rain, temp] = await Promise.allSettled([
    fetchSoil(), fetchNominatim(), fetchOverpass(),
    fetchNDVI(), fetchRainfall(), fetchTemperature(),
  ]);
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("signals.ts written with 3-year trend + area-based APIs!")
